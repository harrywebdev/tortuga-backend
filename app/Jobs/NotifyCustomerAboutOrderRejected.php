...
class NotifyCustomerAboutOrderRejected implements ShouldQueue
{
    ...
    public function handle(CustomerCommunicator $communicator)
    {
        ...
        if ($this->order->status == OrderStatus::REJECTED()) {
            $communicator->sendOrderRejectedNotification($this->order->customer, $this->order);
            return;
        }
    }
}
